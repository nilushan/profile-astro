...
### Framework Stack
- **Astro 5.x** - Main framework in server mode; portfolio pages are prerendered and `/api/chat` uses SSR
...
- **See**: `docs/design-system.md` for current guidance and `docs/project-history/portfolio-improvements.md` for the consolidated improvement history
...
### Scripts
- `pnpm dev` - Development server (hot reload at http://localhost:4321 by default)
- `pnpm build` - Production build (generates `dist/client/` and `dist/server/`)
- `pnpm preview` - Preview production build locally
- `pnpm astro` - Direct access to Astro CLI

### Build System Details
- **Build tool**: Vite (integrated into Astro)
- **Output directories**: `dist/client/` for Hosting and `dist/server/` for SSR
...
- `firebase.json` - Firebase Hosting and Functions config (Hosting public directory: `dist/client/`)
- `.gitignore` - Excludes build output, dependencies, and local environment files

### Development Environment
- **Node version**: 22.x recommended (matches CI and Firebase Functions)
...
- **Steps**:
  1. Checkout code and set up pnpm/Node.js 22
  2. Install dependencies and build
  3. Copy `dist/` to `functions/dist/` and install Functions dependencies
  4. Deploy Firebase Hosting to the live channel
  5. Deploy the `api` Cloud Function
- **Secrets**: `FIREBASE_SERVICE_ACCOUNT_NILUSHANDEVELOPMENT` and `FIREBASE_TOKEN`

#### Preview Deployment (`firebase-hosting-pull-request.yml`)
- **Trigger**: Pull request events (internal PRs only)
- **Workflow**: Builds the project and deploys Hosting only to a preview channel

### Deployment Details
- **Hosting**: Google Firebase Hosting (`nilushandevelopment` project)
- **Build output**: `dist/client/` and `dist/server/`
- **API**: Firebase Cloud Function in `australia-southeast1`
- **Documentation**: `docs/deployment/firebase.md`
...
### Deployment
- Prerendered pages can be hosted statically, but `/api/chat` requires a compatible server adapter
- Firebase serves `dist/client/` and runs the Astro server bundle through Cloud Functions
- Environment-specific configurations are supported

---

**Astro Version**: 5.16.x
**React Version**: 19.x
